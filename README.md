...
<body class="text-rose-800">
    ...
    <section id="hero-landing" class="min-h-screen flex flex-col items-center justify-center text-center p-8 bg-rose-50 textured-bg cursor-pointer transition-opacity duration-1000 ease-out">
        <div class="mb-6">
        ...
        <h1 class="font-serif text-5xl md:text-7xl font-bold text-rose-900 mb-4">¿No sabes dónde encontrar lo que buscas?</h1>
        <p class="mt-4 text-lg text-rose-700 max-w-2xl">Haz clic para explorar nuestra guía completa de proveedores en Santiago.</p>
        <i class="fas fa-arrow-circle-right text-6xl text-rose-400 mt-12 animate-pulse"></i>
    </section>
    ...
    <div id="main-app-content" class="hidden opacity-0 transition-opacity duration-1000 ease-out">
        ...
        <section id="categories-section" class="py-16 sm:py-24 bg-rose-50 textured-bg section-fade-in">
            <div class="container mx-auto px-6">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const initMapAndRender = () => {
            if (!mapInitialized) {
                ...
                // Si el mapa ya está inicializado, invalida su tamaño para que se reajuste si el contenedor cambió
        ...
        const renderCategoriesAndStores = () => {
            ...
            // Ordena las categorías según el array `categoryOrder`
            const sortedCategories = Object.keys(groupedStores).sort((a, b) => {
                const indexA = categoryOrder.indexOf(a);
                const indexB = categoryOrder.indexOf(b);
                if (indexA === -1) return 1; // Coloca las categorías no definidas al final
                if (indexB === -1) return -1;
                return indexA - indexB;
            });

            categoriesContainer.innerHTML = ''; // Limpiar el contenedor antes de renderizar
            sortedCategories.forEach(category => {
                const stores = groupedStores[category];
                const categorySection = document.createElement('div');
                categorySection.className = 'section-fade-in'; // Clase para animación al aparecer
                const iconClass = categoryIcons[category] || 'fa-solid fa-store'; // Icono de respaldo
                ...
                categorySection.innerHTML = `
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        ${stores.map(store => `
                            <div class="bg-white rounded-xl shadow-md flex flex-col group overflow-hidden cursor-pointer h-full" onclick="showStoreDetails(${store.id})">
            ...
            storesData.forEach(store => {
                if (store.lat && store.lon) {
                    const icon = L.divIcon({
                        html: '<i class="fas fa-map-marker-alt text-4xl text-rose-500 drop-shadow-md"></i>',
                    ...
                    const marker = L.marker([store.lat, store.lon], { icon: icon })
                        .addTo(markersLayer)
                        .bindPopup(`
                            <b class="text-rose-800 text-lg">${store.name}</b><br>
                            <span class="text-rose-600">${store.address}</span><br>
                            <button class="text-rose-500 hover:text-rose-700 mt-2 text-sm" onclick="showStoreDetails(${store.id})">Ver detalles</button>
                        `);
            ...
            modalContent.innerHTML = `
                <div class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        <h2 class="font-serif text-3xl font-bold text-rose-900">${store.name}</h2>
                        <button onclick="closeModal()" class="text-rose-400 hover:text-rose-600 text-3xl leading-none">×</button>
                    </div>
                    ...
                    <p class="text-rose-700 mb-6 text-base">${store.description}</p>
                    
                    <div class="border-t border-rose-100 pt-4">
                        <h4 class="font-semibold text-rose-800 mb-3">Contacto</h4>
                        <div class="flex flex-wrap gap-x-6 gap-y-2">
                            ${store.contact.whatsapp ? `<a href="https://wa.me/${store.contact.whatsapp.replace(/\+/g, '')}" target="_blank" class="flex items-center space-x-2 text-rose-600 hover:text-green-500 transition-colors"><i class="fab fa-whatsapp fa-lg"></i><span>WhatsApp</span></a>` : ''}
                            ${store.contact.instagram ? `<a href="https://instagram.com/${store.contact.instagram}" target="_blank" class="flex items-center space-x-2 text-rose-600 hover:text-pink-500 transition-colors"><i class="fab fa-instagram fa-lg"></i><span>Instagram</span></a>` : ''}
                            ${store.contact.web ? `<a href="${store.contact.web}" target="_blank" class="flex items-center space-x-2 text-rose-600 hover:text-blue-500 transition-colors"><i class="fas fa-globe fa-lg"></i><span>Sitio Web</span></a>` : ''}
                        </div>
                    </div>
                    ${store.address !== 'Online' && store.lat && store.lon ? `
                    <div class="mt-6">
                        <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(store.address)}" target="_blank" class="inline-block w-full text-center bg-rose-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-rose-600 transition-colors shadow-md">
                            Ver en Google Maps
                        </a>
                    </div>` : ''}
                </div>`;
        ...
        window.closeModal = () => {
            ...
            setTimeout(() => {
                ...
                modalContent.classList.add('modal-enter'); // Reiniciar para la próxima apertura
        ...
        heroLanding.addEventListener('click', () => {
            heroLanding.classList.add('opacity-0'); // Inicia el fade-out del hero
            setTimeout(() => {
                heroLanding.classList.add('hidden'); // Oculta el hero después del fade-out
                mainAppContent.classList.remove('hidden'); // Muestra el contenedor principal
                
                // Forzar reflow para que el fade-in del contenido principal funcione
                requestAnimationFrame(() => {
                    mainAppContent.classList.add('opacity-100'); // Inicia el fade-in del contenido principal
                });

                initMapAndRender(); // Inicializa el mapa y los marcadores
                renderCategoriesAndStores(); // Renderiza las categorías y tiendas

                // Observa todas las secciones con la clase 'section-fade-in' dentro del contenido principal
                document.querySelectorAll('#main-app-content .section-fade-in').forEach(section => {
                    observer.observe(section);
                });

                // Opcional: Desplazarse suavemente a la sección del mapa al entrar
                document.getElementById('map-section').scrollIntoView({ behavior: 'smooth' });

            }, 1000); // La duración coincide con 'transition-opacity duration-1000'
        });

        // NOTA: No se llaman initMapAndRender() ni renderCategoriesAndStores() al inicio
        // porque se ejecutarán después de que el usuario haga clic en la página de inicio.
    });
    </script>
</body>
</html>
